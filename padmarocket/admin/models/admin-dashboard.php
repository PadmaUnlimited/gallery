...
class PUR_AdminModel {
	...
	function __construct() {
		...
		butler_load( PADMAROCKET_COMPONENTS_PATH . 'api/class' );
		
		$this->url = butler_get( 'page' ) ? admin_url( 'admin.php?page=' . butler_get( 'page' ) ) : '';
		
		$this->refresh = butler_get( 'pur-action' ) == 'refresh' ? true : false;
	...
	function get_components() {
		...
		if ( (isset( $api_request['error'] ) && $api_request['error'] || isset( $api_request['code'] ) && $api_request['code']) && !butler_get( 'pur-notice' ) ) {
			
			wp_redirect( add_query_arg( array( 'pur-notice' => 'request-error', 'error-code' => $api_request['code'] ) , $this->url ) );
	...
	function admin_notices() {
		...
		$action = butler_get( 'pur-notice' );
		
		if ( $action == 'request-error' )
			if ( butler_get( 'error-code' ) == 1 )
				$notice .= '<div id="message" class="error"><p>' . sprintf( __( 'Whoops! It would seem that you are not connected to the internet. Please check your internet connection and try again.', 'Padmarocket' ), butler_get( 'name' ) ) . '</p></div>' . "\n";
			else
				$notice .= '<div id="message" class="error"><p>' . sprintf( __( 'Whoops! Our update server seems to be offline. It’s likely that we’re doing maintainance on the server, so please try again later.', 'Padmarocket' ), butler_get( 'name' ) ) . '</p></div>' . "\n";

		if ( $action == 'component-activated' )
			$notice = '<div id="message" class="success updated fade"><p>' . sprintf( __( '<strong>%s</strong> has been activated successfully.', 'Padmarocket' ),  butler_get( 'name' ) ) . '</p></div>' . "\n";
		
		if ( $action == 'component-activation-error' )
			$notice = '<div id="message" class="error"><p>' . sprintf( __( 'There was an error activating <strong>%s</strong>, please try again.', 'Padmarocket' ), butler_get( 'activated-component' ) ) . '</p></div>' . "\n";
		
		if ( $action == 'component-deactivated' )
			$notice = '<div id="message" class="success updated fade"><p>' . sprintf( __( '<strong>%s</strong> has been deactivated successfully.', 'Padmarocket' ),  butler_get( 'name' ) ) . '</p></div>' . "\n";
						
		if ( $action == 'component-deactivation-error' )
			$notice = '<div id="message" class="error"><p>' . sprintf( __( 'There was an error deactivating <strong>%s</strong>, please try again.', 'Padmarocket' ), butler_get( 'activated-component' ) ) . '</p></div>' . "\n";
		
		if ( $action == 'component-downloaded' )
			$notice = '<div id="message" class="success updated fade"><p>' . sprintf( __( '<strong>%s</strong> has been downloaded successfully.', 'Padmarocket' ),  butler_get( 'name' ) ) . '</p></div>' . "\n";
			
		if ( $action == 'download-component-error' )
			$notice .= '<div id="message" class="error"><p>' . sprintf( __( 'There was an error downloading <strong>%s</strong>, please try again.', 'Padmarocket' ), butler_get( 'name' ) ) . '</p></div>' . "\n";
	...
	function deactivate_component( $slug, $redirect = false ) {
		...
		if ( $redirect ) {
			...
			if ( $deactivated ) {
				...
				wp_redirect( add_query_arg( array( 'pur-notice' => 'component-deactivated', 'name' => $this->components[$slug]['name'] ) , $this->url ) );
				
				exit;
				
			} else {
			
				wp_redirect( add_query_arg( array( 'pur-notice' => 'component-deactivation-error', 'name' => $this->components[$slug]['name'] ) , $this->url ) );
	...
	function activate_component( $slug, $redirect = false, $from_download = false ) {
		...
		if ( $redirect ) {
			...
			if ( $activated ) {
				...
				wp_redirect( add_query_arg( array( 'pur-notice' => $notice, 'name' => $this->components[$slug]['name'] ) , $this->url ) );
	...
	function download_component( $slug, $redirect = false ) {
		...
		if ( $downloaded ) {
			...
			if ( $redirect ) {
				...
				wp_redirect( add_query_arg( array( 'pur-notice' => 'component-downloaded', 'name' => $this->components[$slug]['name']) , $this->url ) );
			...
			if ( $redirect ) {
				...
				wp_redirect( add_query_arg( array( 'pur-notice' => 'download-component-error', 'name' => $this->components[$slug]['name'] ) , $this->url ) );
