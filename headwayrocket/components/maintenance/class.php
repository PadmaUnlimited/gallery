...
class HeadwayRocketBlockMaintenance {
	...
	function elements_mapping( $elements ) {
		...
		if ( !isset( $this->option[$task][$active_skin] ) ) {
			...
			if ( version_compare( HEADWAY_VERSION, '3.4.5', '>=' ) ) {
			    ...
			    $blocks = HeadwayBlocksData::get_blocks_by_type( $this->block_type );
	...
	function merge_default_elements( $defaults_elements ) {
		...
		if ( version_compare( HEADWAY_VERSION, '3.6', '>=' ) && !isset( $this->option[$task] ) ) {
					
			HeadwayElementsData::merge_default_design_data( $defaults_elements, $this->block_type );
