...
;(function($) {
	...
	$(document).ready(function(){
		...
		$(document).on( 'click', 'a.hwr-save', function(e) {
		...
		$(document).on( 'click', 'a.hwr-reload', function(e) {
	...
	hwr.blockOptionsApi = {
		...
		notice:function(block_id, tab, content, show_once) {
			...
			var notice = $('#block-' + block_id + '-tab #sub-tab-' + tab + '-content').find('.hwr-admin-notice');
			...
			notice.find("a.hwr-close").click(function() {
		...
		getLayoutParam:function() {
			...
			if ( this.versionCompare( hwr.headwayVersion, '3.8', '>=' ) )
				return decodeURIComponent( this.getUrlParam('ve-layout') ).split('||');
			else if ( this.versionCompare( hwr.headwayVersion, '3.7', '>=' ) )
